...
defineOptions({
    layout: (p: { currentTeam?: Team | null }) => ({
        breadcrumbs: [
            ...
            {
                title: 'List draft',
                href: salesDraftRoutes.index.url(p.currentTeam!.slug),
            },
        ],
    }),
});
...
const columnVisibility = useStorage<Record<string, boolean>>(
    'sales.drafts.datatable.columns',
...
function visitWithFilters(overrides: Record<string, string | number> = {}) {
    router.get(
        salesDraftRoutes.index.url(teamSlug.value),
...
<template>
    <Head title="List draft" />

    <div class="flex flex-1 flex-col gap-4 p-4 md:p-6">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-2xl font-semibold tracking-tight">List draft</h1>
                ...
                <tbody>
                    ...
                    <tr v-if="!(sales?.data?.length)">
                        ...
                        >
                            ...
                            >
                                <Link
                                    :href="salesDraftRoutes.create.url(teamSlug)"
                                    >Create one</Link
                                >
                            </Button>
                        </td>
