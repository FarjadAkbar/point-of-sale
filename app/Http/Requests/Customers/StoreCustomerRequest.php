...
class StoreCustomerRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            'party_role' => ['required', 'string', Rule::in(['supplier', 'customer', 'both'])],
            'entity_type' => ['required', 'string', Rule::in(['individual', 'business'])],
            'customer_code' => [
                'nullable',
                'string',
                'max:100',
                Rule::unique('customers', 'customer_code')->where('team_id', $team->id),
            ],
            'customer_group_id' => [
                'nullable',
                'integer',
            ...
            'business_name' => [
                Rule::requiredIf($this->input('entity_type') === 'business'),
                'nullable',
                'string',
                'max:255',
            ],
            ...
            'first_name' => [
                Rule::requiredIf($this->input('entity_type') === 'individual'),
                'nullable',
                'string',
                'max:255',
            ],
